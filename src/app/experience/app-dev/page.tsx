...
type SortOption = "updated-desc" | "updated-asc" | "created-desc" | "created-asc";
...
function sortExperiences(list: typeof experiences, sort: SortOption) {
...
export default function AppDevExperiencePage() {
  ...
  const filtered = useMemo(() => {
    ...
    return sortExperiences(list, sort);
  }, [query, sort]);
    ...
    <div className="px-5 pt-3 pb-6">
      ...
      <div className="relative mb-2 flex justify-end">
        <button
          ...
          className="flex items-center gap-1.5 text-xs text-neutral-500 bg-neutral-100 px-3 py-1.5 rounded-full"
        >
          <ArrowUpDown size={13} />
          {sortLabels[sort]}
        </button>
        {showSortMenu && (
          <>
            ...
            <div className="absolute top-8 right-0 z-50 bg-white border border-neutral-100 rounded-xl shadow-lg py-1 min-w-[160px]">
      ...
      <div className="-mx-5">
        {filtered.map((exp, i) => (
          <article key={exp.slug}>
            ...
            <Link href={`/experience/app-dev/${exp.slug}`} className="block px-5 py-5">
              ...
              <div className="text-right mt-4">
                <p className="text-[11px] text-neutral-400">수정 {exp.updatedDate}</p>
                <p className="text-[11px] text-neutral-300 mt-0.5">생성 {exp.createdDate}</p>
              </div>
            </Link>
          </article>
        ))}
